...
def handle_CREMA_D(directory):
    ...
    for i, file in enumerate(os.listdir(directory)):
        ...
        intensity.append(file.split("_")[3])

    # Creating a csv file with the extracted labels
    labels = pd.DataFrame(list(zip(index, speaker, line, emotions, intensity)), columns = ["Index", "filename", "Speaker", "Line", "Emotion", "Intensity"])
    labels.to_csv(os.path.join(directory, "_labels.csv"), index=False)

    return os.path.join(directory, "_labels.csv")
...
def main():
    ...
    match dataset:
        ...
        case "CREMA-D":
            ...
            labels_path = handle_CREMA_D(audio_directory)
            corrected_labels_path = os.path.join(os.path.dirname(labels_path), os.path.basename(labels_path)[:-4] + "_corrected.csv")
