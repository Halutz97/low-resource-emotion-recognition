...
class CustomWav2Vec2ForSequenceClassification(Wav2Vec2ForSequenceClassification):
    def __init__(self, config, num_linear_layers=1):
        ...
        # Setup the classifier exactly as during training
        layers = [torch.nn.Linear(config.hidden_size, config.hidden_size) for _ in range(num_linear_layers - 1)]
        layers += [torch.nn.Linear(config.hidden_size, config.num_labels)]
        dropout_rate = getattr(config, 'hidden_dropout_prob', 0.1)  # Ensure this matches training
        self.classifier = torch.nn.Sequential(*layers, torch.nn.Dropout(dropout_rate))
...
config = Wav2Vec2Config.from_pretrained("facebook/wav2vec2-large-xlsr-53", num_labels=7, hidden_dropout_prob=0.1)
config.hidden_size = 768  # Ensure this matches your training setup
